...
metrics.postPulsePeakAbsDtheta = NaN;
metrics.thetaZeroDtheta = NaN;
metrics.recoveryTauSaturationRatio = NaN;
metrics.recoveryULqrSaturationRatio = NaN;
...
if isfield(caseDef, "pulseWindow") && all(isfinite(caseDef.pulseWindow))
    pulseEnd = caseDef.pulseWindow(2);
...
postPulse = signals.time >= pulseEnd;
if any(postPulse)
    metrics.postPulsePeakAbsDtheta = max(abs(dtheta(postPulse)));
end

thetaZeroTime = firstThetaZeroCrossing(signals.time, theta, pulseEnd);
if isfinite(thetaZeroTime)
    metrics.thetaZeroDtheta = abs(sampleAt(signals.time, dtheta, thetaZeroTime));
end

postTau = signals.tau.time >= pulseEnd;
if any(postTau)
    metrics.recoveryTauSaturationRatio = max(abs(tau(postTau, :)) ./ ...
        ctrl.tauMax(:).', [], "all");
end

postULqr = signals.uLqr.time >= pulseEnd;
if any(postULqr)
    metrics.recoveryULqrSaturationRatio = max(abs(uLqr(postULqr, :)) ./ ...
        [base.forceMax(:).', base.momentMax], [], "all");
end
...
postPulsePeakAbsDtheta = zeros(n, 1);
thetaZeroDtheta = zeros(n, 1);
recoveryTauSaturationRatio = zeros(n, 1);
recoveryULqrSaturationRatio = zeros(n, 1);
...
for idx = 1:n
    ...
    postPulsePeakAbsDtheta(idx) = m.postPulsePeakAbsDtheta;
    thetaZeroDtheta(idx) = m.thetaZeroDtheta;
    recoveryTauSaturationRatio(idx) = m.recoveryTauSaturationRatio;
    recoveryULqrSaturationRatio(idx) = m.recoveryULqrSaturationRatio;
end
...
summary = table(names, weight, initialPitchDeg, pulseAmplitudeN, stable, ...
    ...
    legPositionRms, legVelocityRms, postPulsePeakAbsDtheta, ...
    thetaZeroDtheta, recoveryTauSaturationRatio, ...
