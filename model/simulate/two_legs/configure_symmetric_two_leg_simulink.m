...
wireSixDofPlanarState(subsystem);
...
coupledQp = add_block(upperController, subsystem + "/Coupled QP", ...
    "MATLABFcn", "coupled_two_leg_qp_signal", "OutputDimensions", "43", ...
...
coupledSplit = add_block("simulink/Signal Routing/Demux", ...
    ...
    "Outputs", "[3 3 3 3 1 1 2 2 1 1 3 3 3 2 2 6 1 1 1 1]", ...
...
function wireSixDofPlanarState(parent)
...
set_param(joint, ...
    ...
    "SphPositionTargetQuaternion", ...
    "[cos(base.x0(3)/2) 0 0 sin(base.x0(3)/2)]", ...
    "SphVelocityTargetSpecify", "on", ...
    "SphVelocityTargetValue", "[0 0 base.x0(6)]", ...
...
rollRateTerminator = add_block("simulink/Sinks/Terminator", ...
    parent + "/Base Roll Rate Terminator", ...
    "Position", [-190, 85, -170, 105]);
yawRateTerminator = add_block("simulink/Sinks/Terminator", ...
    parent + "/Base Yaw Rate Terminator", ...
    "Position", [-190, 130, -170, 150]);
...
connect(parent, angularVelocitySplit, 1, rollRateTerminator, 1);
connect(parent, angularVelocitySplit, 2, yawRateTerminator, 1);
...
suffixes = ["Slack", "Feasible", "Slack Norm", "QP Status", ...
    ...
    "Dynamics Residual", "Differential Qdd", ...
