...
for k = 1:numel(cases)
    ...
    results(k).maxContactResidualDirection = max(abs(qp(:, 49:51)), [], 1);
    results(k).maxWrenchSlackNorm = max(qp(:, 31));
    results(k).maxAbsWrenchSlack = max(abs(qp(:, 7:18)), [], 1);
    results(k).maxAbsTorque = max(abs(qp(:, 1:6)), [], 1);
    results(k).maxAbsContactForce = max(abs(qp(:, 33:38)), [], 1);
    results(k).finalXiCommonError = qp(end, 52);
    results(k).maxAbsXiCommonError = max(abs(qp(:, 52)));
    results(k).maxAbsDxiCommonError = max(abs(qp(:, 53)));
    results(k).maxWheelTaskResidual = [max(qp(:, 60)), ...
        max(abs(qp(:, 73) - qp(:, 74)))];
    results(k).meanQpSolveTime = mean(qp(:, 54));
    results(k).maxQpSolveTime = max(qp(:, 54));
    results(k).simulationWallTime = simulationWallTime;
    ...
    fprintf("%s: final [x z pitch] = [%.4g %.4g %.4g deg], " + ...
        ...
        "max |xiDelta| %.3g m, slack %.3g, QP/NMPC max %.3g/%.3g ms.\n", ...
        cases(k).name, ...
        ...
        results(k).maxAbsXiDelta, results(k).maxWrenchSlackNorm, ...
        1e3*results(k).maxQpSolveTime, 1e3*results(k).maxNmpcCpuTime);
end
clear cleanup
end
